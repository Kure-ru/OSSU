...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Self-Ref-P5-largest) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; ListOfNumber -> Number
;; produces the largest number or 0 if list is empty
(check-expect (largest-num empty) 0)
(check-expect (largest-num (cons 23 empty)) 23)
(check-expect (largest-num LON2) 60)
...
(define (largest-num lon)
  (cond [(empty? lon) 0]
        [else
         (if (> (first lon) (largest-num (rest lon)))
             (first lon)
