...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname Accumulators-P10-rev-tr) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (rev lox0)
  (local [(define (rev lox acc)                   
            (cond [(empty? lox) acc]
                  [else (rev (rest lox) (cons (first lox) acc))]))]
    (rev lox0 empty)))