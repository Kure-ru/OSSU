...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname Graphs-P4-max-exits-from) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
(define (max-exits-from r0)
  ...
  ;; greatest-room is the room with the greatest number of exits found so far
  (local [(define (fn-for-room r todo visited greatest) 
            (if (member (room-name r) visited)
                (fn-for-lor todo visited greatest)
                (fn-for-lor (append (room-exits r) todo)
                            (cons (room-name r) visited)
                            (max-exits r greatest))))
          (define (fn-for-lor todo visited greatest)
            (cond [(empty? todo) greatest]
                  [else
                   (fn-for-room (first todo) 
                                (rest todo)
                                visited
                                greatest)]))
          (define (max-exits r greatest)
            (if (>= (length (room-exits greatest))
                    (length (room-exits r)))
                greatest
                r))]
