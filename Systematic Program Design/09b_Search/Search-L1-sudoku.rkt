...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname Search-L1-sudoku) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (valid-board? bd)
  (local [(define (valid-units? lou) ;(listof Unit) -> boolean
          ...
          (define (read-pos p)     ;Pos -> Val|false
            (read-square bd p))    ;produces contents of bd at p
            ...
            (cond [(empty? lov) true]
                  [else
                   (if (member (first lov) (rest lov))
                       false
                       (no-duplicates? (rest lov)))]))]   ;produce true if no value in lov appears twice
    (valid-units? UNITS)))
