...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Helpers-P1-render-roster) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; produce a list of player that is on a team t
...
(check-expect (on-team? (make-player "Samael" 1) 1) true)
(check-expect (on-team? (make-player "Georgia" 2) 1) false)
...
(define (render-roster lop)
  (beside/align "top" (render-team (select-players lop 1) 1)
...
(check-expect (render-team (cons (make-player "Mike" 2) (cons (make-player "Georgia" 2) empty)) 2) (above (render-cell "Team 2")
                                                                                                          (render-cell "Mike")
                                                                                                          (render-cell "Georgia")))
...
(define (render-team lop t)
  (cond [(empty? lop) (render-cell(string-append "Team " (number->string t)))]
        [else
         (above
          (render-cell(string-append "Team " (number->string t)))
...
(check-expect (player-names LOP2) (cons "Samael" (cons "Georgia" empty)))
